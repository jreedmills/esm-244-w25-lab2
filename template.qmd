...
1. Randomly group the data into 10 equal groups
2. Select one group to be the test group, and use the remaining data to train the model
3. Make prediction on the test set with fitted training model
4. Repeat steps 2-3 for each group
5. Repeat steps 1-4 for each model
6. Compare root mean squared error for each model and select the best one
7. Create final model built on all data
...
#Without replacing, you will run out of points after sampling 5 time
...
fold_vec<-rep(1:folds,length.out=nrow(penguins_clean)) #create a vector repeating 1 to 10 until you reach the number of rows in the penguins dataset

penguins_fold<-penguins_clean |> 
  mutate(group=sample(fold_vec,size=n(),replace=FALSE)) #new column that randomly samples the 1:10 vector without replacing
